...
////size:字体大小 12/16/24
////mode:叠加方式(1)还是非叠加方式(0)
//LCD_ShowchineseChar(0,0, u8 num, u8 size, u8 mode)
...
/*
    ...
    @plcd 
*/
void Draw_word(int x,int y,int w,int h,int color,const char s[])
{
    //1.遍历字模数组的元素
    int i,j;
    ...
    for(i=0;i < w*h/8;i++)
    {
        ...
        for(j=7;j>=0;j--)
        {
            ...
            if((s[i]>>j)&1)
            {
							LCD_Fast_DrawPoint(x+8*(i%line_num)+7-j,y+i/line_num,color);
